...
class ReportSectionWrapper extends ConsumerWidget {
  const ReportSectionWrapper({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return const ReportSectionStateful();
  }
}

class ReportSectionStateful extends StatefulWidget {
  const ReportSectionStateful({super.key});

  @override
  State<ReportSectionStateful> createState() => _ReportSectionStatefulState();
}

class _ReportSectionStatefulState extends State<ReportSectionStateful> {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: _getFilteredDateLabels(reportData.dateLabels, showWeekly)
                .map((day) => Flexible(
                      child: Text(
                        day,
                        style: AppTypography.caption1.copyWith(
                    ...
                    child: CupertinoButton(
                      ...
                      onPressed: () {
                        showWeightInputDialog(
                          context: context,
                          currentWeight: hasWeightData ? reportData.weightData.last : null,
  ...
  List<String> _getFilteredDateLabels(List<String> dateLabels, bool showWeekly) {
    if (showWeekly) {
      return dateLabels;
    } else {
      // Für 30 Tage: Zeige nur jeden 5. Tag um Mo-So zu behalten
      final filtered = <String>[];
      for (int i = 0; i < dateLabels.length; i += 5) {
        filtered.add(dateLabels[i]);
      }
      return filtered;
    }
  }
